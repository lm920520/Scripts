--[[
	Pantheon WomboCombo 1.0 by WomboCombo
		
		It requires AllClass and Spell Damage Library

	-Full combo: items -> W -> Q -> E -> R
...
function OnLoad()
	MCConfig = scriptConfig("Pantheon WomboCombo 1.1", "Pantheoncombo")
	MCConfig:addParam("scriptActive", "Combo", SCRIPT_PARAM_ONKEYDOWN, false, 32)
	MCConfig:addParam("harass", "Harass", SCRIPT_PARAM_ONKEYDOWN, false, 88)
...
function OnTick()
	...
	if MCConfig.harass then
	if ts.target and not MCConfig.scriptActive then
	if QREADY and GetDistance(ts.target)<=600  then CastSpell(_Q, ts.target) end
end
end
					--end
		if MCConfig.scriptActive then
			if ts.target then
				...
				if WREADY and GetDistance(ts.target)<=580 and QREADY and EREADY then CastSpell(_W, ts.target) end
					if EREADY and GetDistance(ts.nextPosition)<=600 and WREADY == false and QREADY == false then CastSpell(_E, ts.nextPosition.x, ts.nextPosition.z) end
					if QREADY and GetDistance(ts.target)<=600 and WREADY == false then CastSpell(_Q, ts.target) end
					if doult == true then
					if ts.target then
					if GetDistance(ts.target)>=2000 then
		     if RREADY and GetTickCount()-lastr>=2.0 and GetDistance(ts.nextPosition)<=5500  then CastSpell(_R, ts.nextPosition.x, ts.nextPosition.z)  doult = false end
				 end
				 end
				 end
					end
...
function MCDmgCalculation()
	...
	if ValidTarget(enemy) then
		...
		local combo1 = hitdamage + qdamage + wdamage + pdamage + rdamage + Sheendamage + Trinitydamage + LichBanedamage --0 cd
		...
		elseif combo3 >= enemy.health then killable[calculationenemy] = 3  doult = true
		elseif combo2 >= enemy.health then killable[calculationenemy] = 2 doult = true
		elseif combo1 >= enemy.health then killable[calculationenemy] = 1 doult = false
		else killable[calculationenemy] = 0 doult = false end
	end
...
function OnDraw()
	if MCConfig.drawcircles and not myHero.dead then
		...
		DrawCircle(myHero.x, myHero.y, myHero.z, range3, 0x19A712)
		--- make yellow 2000 ring here 
		...
		if ValidTarget(enemydraw) then
			if MCConfig.drawcircles then
				...
				elseif killable[i] == 2 then
					...
					if doult == true then
        DrawText("Do Ult",25,500,80,0xFFFF66FF)
				end
...
PrintChat(" >> Pantheon WomboCombo 1.0 loaded!")