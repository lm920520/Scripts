...
-- #                                  Most credit goes to Sida for making XT002-HUD                   # --
-- #################################################################################################### --
...
--[[LIST OF SCRIPTS IN THIS SCRIPT:
	...
	Pro CD Tracker <-- Off by default(After you tick it On in menu, Press F9 two times in game SLOWLY to turn On)
    Immune Countdown
    Life Saver <-- Off by default(After you tick it On in menu, Press F9 two times in game SLOWLY to turn On)
        >>ADDED NOTES FOR LIFE SAVER: If you get one of the items it supports, make sure to reload(F9) so the menu will show for that item.
...
class 'ImmuneCountdown'

ImmuneCountdown._immuneEffects = {
...
function ImmuneCountdown:__init()
    self._immuneTable = {}
...
function ImmuneCountdown:_OnCreateObj(object)
    if object and object.valid then
        for _, effect in pairs(ImmuneCountdown._immuneEffects) do
            if effect[1] == object.name then
                ...
                self._immuneTable[nearestHero.networkID] = os.clock() + effect[2]
            end
        end
    end
end

function ImmuneCountdown:_OnDraw()
    for networkID, time in pairs(self._immuneTable) do
        local unit = objManager:GetObjectByNetworkId(networkID)
            
        if unit and unit.valid and not unit.dead and GetDistanceSqr(myHero, unit) <= self._checkDistance then
            DrawText3D(tostring(math.round(time - os.clock())), unit.x, unit.y, unit.z, 70, RGB(255, 255, 255), true)
        end
    end
end

function ImmuneCountdown:_ClearImmuneTable()
    for networkID, time in pairs(self._immuneTable) do
...
-- function Console__OnMsg1(msg, key)
    -- local messageBoxHeight = 2 * console.padding + (maxMessages - 1) * (console.textSize + console.linePadding) + console.textSize
    -- local promptHeight         = 2 * console.padding + console.textSize
    -- local consoleHeight        = messageBoxHeight + promptHeight
    -- local scrollbarHeight    = math.ceil(messageBoxHeight / math.max(#stack / maxMessages, 1))
...
-- ###############################################################################
    ...
    KCConfig:addParam("WhereHeGo", "Where Did He Go? v0.4", SCRIPT_PARAM_ONOFF, true)
    ...
    KCConfig:addParam("AwesomeScript", "Awesome Script", SCRIPT_PARAM_ONOFF, false) -- <-- Off by default (RELOAD IN GAME FOR NORMAL RANGE COLORFUL)
    KCConfig:addParam("ProTrack", "Pro Tracker", SCRIPT_PARAM_ONOFF, false) -- <-- Off by default
    KCConfig:addParam("ImmuneCount", "Immune Countdown", SCRIPT_PARAM_ONOFF, true)
    KCConfig:addParam("LifeSaver", "Life Saver v2.1 (Reload(F9) if error)", SCRIPT_PARAM_ONOFF, false) -- <-- Off by default
    
    if KCConfig.LifeSaver then
        LifeSaverOnLoad()
    end
    if KCConfig.ImmuneCount then
        ICDOnLoad()
    end
    if KCConfig.ProTrack then
        ProTOnLoad()
    end
