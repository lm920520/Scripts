...
local FizzQ = {range = 550 , width = 0 , speed = math.huge, delay = 0.5}
local FizzW = {range = 0 , width = 0 , speed = 0, delay = 0.5}
local FizzE = {range = 400 , width = 330 , speed = 1300, delay = 0.5}
local FizzE2 = {range = 400 , width = 270 , speed = 1300, delay = 0.5}
local FizzR = {range = 1275 , width = 250 , speed = 1200, delay = 0.5}
   ...
   -- if updateCheck == false then
       ...
       -- if latestVersion~=nil and latestVersion ~= VERSION then
           ...
           -- DownloadFile(URL, PATH,function ()
            ...
            -- end)
        -- ecassio4everlseif latestVersion == VERSION then
            -- updateCheck = true
...
function OnLoad()
  ...
  --Q
    Q = Spell(_Q, FizzQ.range)
    --W
    W = Spell(_W, FizzW.range)
    --E
    E = Spell(_E, FizzE.range)
    --R
    R = Spell(_R, FizzR.range)
--VPred Settings
R:SetSkillshot(VP, SKILLSHOT_LINEAR, FizzR.width, FizzR.delay, FizzR.speed, true)

E:SetSkillshot(VP, SKILLSHOT_CIRCULAR, FizzE.width, FizzE.delay, FizzE.speed, false)
...
--Damage Calculator
    DLib:RegisterDamageSource(_Q, _MAGIC, -20, 30, _MAGIC, _AP, 0.60, function() return (player:CanUseSpell(_Q) == READY) end)
    DLib:RegisterDamageSource(_W, _MAGIC, 20, 10, _MAGIC, _AP, 0.35, function() return (player:CanUseSpell(_W) == READY) end)
...
function Combo()
	...
	ItemActivator:UseItem("Zhonya")
	ItemActivator:UseItem("DFC", Qtarget)
