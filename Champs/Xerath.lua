...
--Spell Data
local Ranges = {[_Q] = {750, 1400},  [_W] = 1100, [_E] = 1050, [_R] = {3200, 4400, 5600}}
local Widths = {[_Q] = 100, [_W] = 200, [_E] = 70, [_R] = 200}
local Delays = {[_Q] = 0.75, [_W] = 0.5, [_E] = 0.5, [_R] = 0.75}
local Speeds = {[_Q] = 500, [_W] = 20, [_E] = 1600, [_R] = 500}
local RangeCircles = {}

--Passive Tracking
local PassiveUp = true
local LastPing = 0
local InCharge = false
...
function OnLoad()
	...
	Q:SetCharged("XerathArcanopulseChargeUp", 3, Ranges[_Q][2], 1.5, GetCh())
	Q:SetAOE(true)
...
function GetCh()
	local targ = STS:GetTarget(Q.range)
	if target then
		local castPosition, hitChance, nTargets = Q:GetPrediction(QTarget)
		if GetDistance(castPosition) < Q.range and Q:IsCharging() then
			return true
		end
	end
	return false
...
-- function PrintCharge()
	-- if InCharge == true then
		-- print("InCharge = true")
	-- else
		-- print("InCharge = false")
	-- end
-- end
-- TickLimiter(PrintCharge, 1)
...
function Combo()
	...
	if QTarget and Menu.Combo.UseQ then
		...
		if Q:IsCharging() then
			...
			if GetDistance(castPosition) < (Q.range - 50) then
				Cast2Q(castPosition)
				print("Cast 2 Q")
				-- InCharge = false
			end
...
function OnDraw()
	...
	DrawText("My Range = "..Q.range, 17, 100, 100, ARGB(255,0,255,0))
	if InCharge == true then
		DrawText("IsCharging = true", 17, 100, 120, ARGB(255,0,255,0))
	else
		DrawText("IsCharging = false", 17, 100, 120, ARGB(255,0,255,0))
	end
end
