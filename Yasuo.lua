...
function getVersion()
 GetAsyncWebResult("dl.dropboxusercontent.com","/u/22305037/Yasuo.txt",getDownloadVersion)
end 

-- local latestVersion = getVersion()
-- print(tostring(latestVersion))
-- function update()
--     if updateCheck == false then
--         local PATH = BOL_PATH.."Scripts\\Yasuo.lua"
--         local URL = "https://dl.dropboxusercontent.com/u/22305037/Yasuo.lua"
...
if autoUpdate then
    SourceUpdater(scriptName, version, "dl.dropboxusercontent.com", "/u/22305037/Yasuo.lua", SCRIPT_PATH .. GetCurrentEnv().FILE_NAME):SetSilent(silentUpdate):CheckUpdate()
end
...
function OnLoad()
  ...
  ScriptSetUp()
  ScriptSetUp1()
  PrintChat("<font color=\"#0DF8FF\">AWA and PYR Yasuo loaded</font> <font color=\"#FF0000\">"..tostring(version).."</font> <font color=\"#0DF8FF\">Successfully</font> ")
...
function Combo()
  ...
  if itarget then
    ...
    if Qfound then AutoQ() end
    if RDQ == false  then
        if (Qfound and  Q:IsReady() and Config.ComboSub.ComboSub1.useQ and not Config.ComboSub.Packet.useP and not isCastingE) or (Q:IsReady() and Config.ComboSub.ComboSub1.useQ and isCastingE and TS:GetTarget(250)) then
      Q:Cast(Qfound)
      elseif (Qfound and  Q:IsReady() and Config.ComboSub.ComboSub1.useQ and Config.ComboSub.Packet.useP and not isCastingE) or (Q:IsReady() and Config.ComboSub.ComboSub1.useQ and isCastingE and TS:GetTarget(250)) then
        CastQPacket(Qfound)
        end
    end
    if RDQ == true then
      if (QEfound and  Q:IsReady() and Config.ComboSub.ComboSub1.useQ and not Config.ComboSub.Packet.useP and not isCastingE) or (Q:IsReady() and Config.ComboSub.ComboSub1.useQ and isCastingE and TS:GetTarget(250)) then
        Q:Cast(QEfound)
        elseif (QEfound and  Q:IsReady() and Config.ComboSub.ComboSub1.useQ and  Config.ComboSub.Packet.useP and not isCastingE) or (Q:IsReady() and Config.ComboSub.ComboSub1.useQ and isCastingE and TS:GetTarget(250)) then
        CastQPacket(QEfound)
        end
    end
    if Efound and  E:IsReady() and Config.ComboSub.ComboSub1.useE  then
      if Config.OOSub.nodive then
          if not UnderTurret(Efound) then
            E:Cast(Efound)
            isCastingE = true
          end
      elseif not Config.OOSub.nodive then
          E:Cast(Efound)
          isCastingE = true
      end
    end
...
--((MMA))--
    ...
    if MMATarget ~= nil then
        if (Q:IsReady() and Config.ComboSub.ComboSub1.useQ and not Config.ComboSub.Packet.useP and not isCastingE) or (isCastingE and TS:GetTarget(250)) then
        Q:Cast(MMATarget)
        elseif (Q:IsReady() and Config.ComboSub.ComboSub1.useQ and  Config.ComboSub.Packet.useP and not isCastingE) or (isCastingE and TS:GetTarget(250)) then
...
function AutoQ()
  ...
  local QEfound = TS:GetTarget(SpellQEmpowered.Range)
    if not (Config.OOSub.nodive and UnderTurret(myHero.pos)) and not RecallDetection then
        if RDQ == false then
          if Qfound and Q:IsReady() and Config.ASub.useQ and not Config.ComboSub.Packet.useP and not isCastingE then
            Q:Cast(Qfound)
          elseif (Qfound and  Q:IsReady() and Config.ASub.useQ and Config.ComboSub.Packet.useP and not isCastingE) or (TS:GetTarget(250) and Q:IsReady() and Config.ASub.useQ and not Config.ComboSub.Packet.useP) then
            CastQPacket(Qfound)
            --print('autoq')
          end
        end
    end
    if RDQ == true and not RecallDetection then
        if QEfound and  Q:IsReady() and Config.ASub.useQ2 and not Config.ComboSub.Packet.useP and not isCastingE then
          Q:Cast(QEfound)
        elseif (Qfound and  Q:IsReady() and Config.ASub.useQ and Config.ComboSub.Packet.useP and not isCastingE) or (TS:GetTarget(250) and Q:IsReady() and Config.ASub.useQ and not Config.ComboSub.Packet.useP) then
          CastQPacket(QEfound)
        end
 ...
 if Target ~= nil then 
  if not RDQ then
    ...
    local CastPosition, HitChance, Position =  VP:GetLineCastPosition(Target, SpellQ.Delay , SpellQ.Width , SpellQ.Range , SpellQ.Speed , myHero, false)
      if HitChance >= 1 and not isCastingE then
  ...
  if RDQ then
    ...
    if HitChance >= 2 and not isCastingE then
        Packet("S_CAST", {spellId = _Q, toX=CastPosition.x, toY=CastPosition.z, fromX=CastPosition.x, fromY=CastPosition.z}):send()
    end
    end
end
  ...
  --print('farm2')
      ...
      if ValidTarget(minion) then
        ...
        if GetDistance(minion, myHero) < SpellQ.Range then
          ...
          if DamageCalculator:CalcSpellDamage(minion,_Q) >= minion.health then
            if (isCastingE == false) or (isCastingE and (GetDistance(minion, myHero) <= 250)) then
    ...
    if Config.FSub.JFarm then
      ...
      for _, minion1 in pairs (JungleMinions.objects) do
        if GetDistance(minion1, myhero) < SpellQ.Range and not isCastingE then
...
--((OnTick))--
function OnTick()
    --if not myHero:CanUseSpell(_E) == READY then
    if isCastingE == true then
      DelayAction(function() isCastingE = false end, 1)
    end
    --if isCastingE == true then PrintChat ("isCastingE") end
    local itarget = TS:GetTarget(600)
    if Loaded then
...
function OnProcessSpell(unit, spell)
    if unit.isMe then
      --print(tostring(spell.name))
        if spell.name == "YasuoDashWrapper" then
        --PrintChat("ECAST")
        --lastTickQ = GetTickCount()
        isCastingE = true
        --DelayAction(function() isCastingE = false end, 1)
      end
    end
